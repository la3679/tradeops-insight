...
- Docker with Compose for infrastructure-backed phases; the service skeleton does not require Docker
...
The API and web checks are reproducible without containers. Compose and migration verification are added when the persistence layer lands. If Docker is unavailable, report container-backed checks as not run rather than treating them as passed.
